...
plot_roc <- function(test_data, prob_matrix, model_name) {
  ...
  plots <- lapply(class_names, function(class_name) {
    class_data <- roc_data[roc_data$Class == class_name, ]
    
    ggplot(data = class_data, aes(x = Specificity, y = Sensitivity)) +
      ...
      annotate("text", x = 0.5, y = 0.1, 
               label = paste("AUC:", round(auc(roc(test_data$FTR == class_name, prob_matrix[, which(class_names == class_name)])), 3)), 
               size = 5, hjust = 0.5)
  })
...
combine_metrics <- function(roc_metrics_list) {
  # Bind the list of data frames into one
  metrics <- do.call(rbind, roc_metrics_list)
  rownames(metrics) <- colnames(prob_multinom_model)  # Set class names as row names
  return(metrics)
}
...
```{r}
# Extract metrics for each class from the confusion matrix
...
# Combine these metrics into a single data frame for each class
...
# Combine with the existing comparison_df
comparison_df <- rbind(
  comparison_df,  
  metrics_stepwise
)
...
metrics_ridge <- data.frame(
  ...
  Class = c("Away Win", "Draw", "Home Win"),
  Accuracy = c(accuracy_ridge, accuracy_ridge, accuracy_ridge),  # Assuming accuracy is the same for all classes
...
```{r}
# Generate the confusion matrix for LDA model
...
```{r}
# Extract metrics for each class from the LDA confusion matrix
...
metrics_lda <- data.frame(
  ...
  Class = c("Away Win", "Draw", "Home Win"),
  Accuracy = c(accuracy_lda, accuracy_lda, accuracy_lda),  # Assuming accuracy is the same for all classes
...
# Plot the ROC curve for the LDA model
...
metrics_nb <- data.frame(
  ...
  Class = c("Away Win", "Draw", "Home Win"),
  Accuracy = c(accuracy_nb, accuracy_nb, accuracy_nb),  # Assuming accuracy is the same for all classes
...
```{r}
saveRDS(multinom_model, "./model/model_file.rds")
```