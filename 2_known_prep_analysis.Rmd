...
# Extract the best threshold and metrics for Stepwise Model
roc_metrics_stepwise <- coords(roc_stepwise, x = "best", ret = "all")

# Rename the row for clarity
row.names(roc_metrics_stepwise) <- "Stepwise Model"

roc_results <- rbind(
  roc_results, 
  roc_metrics_stepwise 
)

# Print the updated roc_results
print(roc_results)
...
```{r}
# Use the same formula as training data
x_test <- model.matrix(FTR ~ ., data = test_data)[, -1]

# Check dimensions of x and x_test
print(dim(x))
print(dim(x_test))

test_data$FTR <- factor(test_data$FTR, levels = levels(train_data$FTR))

# Match test_data columns to training data
x_test <- model.matrix(FTR ~ ., data = test_data, xlev = attr(x, "assign"))[, -1]

# Identify missing columns
missing_cols <- setdiff(colnames(x), colnames(x_test))
for (col in missing_cols) {
  x_test <- cbind(x_test, 0)  # Add a column with all zeros
...
# Repeat for other factor predictors:
...
# Predict probabilities for each class using the best lambda
model_fit.lasso.prob <- predict(
  model_fit.lasso, 
...
prob_matrix <- model_fit.lasso.prob[, , 1]

# Create an empty list to store ROC curves
roc_curves <- list()

# Loop over each class to calculate and plot the ROC curve
for (class in colnames(prob_matrix)) {
  # Create binary response for the current class
  binary_response <- ifelse(test_data$FTR == class, 1, 0)
  
  # Access probabilities for the current class
  class_probs <- prob_matrix[, class]
  
  # Compute the ROC curve
  roc_curve <- roc(binary_response, class_probs)
  roc_curves[[class]] <- roc_curve
  
  # Plot ROC curve for this class
  plot.roc(
    roc_curve, 
    main = paste("ROC Curve - Class", class), 
    col = which(colnames(prob_matrix) == class), 
    lwd = 2,
    print.auc = TRUE
  )
  
  # Calculate sensitivity, specificity, and accuracy
  thresholds <- coords(roc_curve, "best", ret = c("threshold", "sensitivity", "specificity"), best.method = "closest.topleft")
  
  # Print metrics
  cat("Class:", class, "\n")
  cat("Threshold:", thresholds$threshold, "\n")
  cat("Sensitivity:", thresholds$sensitivity, "\n")
  cat("Specificity:", thresholds$specificity, "\n")
  
  # Calculate predicted classes based on the highest probability
  predicted_classes <- apply(prob_matrix, 1, which.max)  # Get the index of the max probability
  actual_classes <- as.numeric(factor(test_data$FTR, levels = colnames(prob_matrix)))  # Convert actual classes to numeric
...
predicted_classes <- apply(prob_matrix, 1, function(x) colnames(prob_matrix)[which.max(x)])

# Convert actual classes to numeric form (same order as the columns in prob_matrix)
actual_classes <- test_data$FTR

# Create confusion matrix
conf_matrix <- confusionMatrix(factor(predicted_classes, levels = colnames(prob_matrix)), factor(actual_classes, levels = colnames(prob_matrix)))
...
```{r}
ridge_coefficients <- coef(model_fit.ridge, s = modelcv_bestlam.ridge)
...
model_fit.ridge.pred <- predict(model_fit.ridge, s = modelcv_bestlam.ridge, 
                                 newx = x_test, type = "response")

# Check the structure of the predictions
str(model_fit.ridge.pred)

# Prepare the response variable
y_test <- factor(test_data$FTR, levels = levels(train_data$FTR))

# Check lengths
cat("Length of y_test:", length(y_test), "\n")
cat("Length of model_fit.ridge.pred:", dim(model_fit.ridge.pred)[1], "\n")  # Use dim for 3D array

# Check contents
print(head(y_test))

# Initialize a list to store ROC curves and thresholds
roc_curves <- list()
best_thresholds <- list()

# Loop through each class to compute ROC
for (class in levels(y_test)) {
    # Extract predicted probabilities for the current class
    class_index <- which(colnames(model_fit.ridge.pred) == class)
    model_fit.ridge.pred_class <- model_fit.ridge.pred[, class_index, 1]
    
    # Compute the ROC curve
    model_fit.ridge.roc <- roc(y_test, model_fit.ridge.pred_class, plot = TRUE, print.auc = TRUE, 
                               main = paste("ROC Curve for", class))
    
    # Get the best threshold
    best_threshold <- coords(model_fit.ridge.roc, "best", ret = "threshold")
    best_thresholds[[class]] <- best_threshold
    roc_curves[[class]] <- model_fit.ridge.roc
}
...
predicted_classes <- apply(model_fit.ridge.pred, 1, which.max)  # Get the index of the max probability
predicted_classes <- levels(y_test)[predicted_classes]  # Convert indices to class labels
...
sensitivity <- numeric(length(levels(y_test)))
specificity <- numeric(length(levels(y_test)))

for (i in seq_along(levels(y_test))) {
    class <- levels(y_test)[i]
    ...
    sensitivity[i] <- TP / (TP + FN)  # Sensitivity (True Positive Rate)
    specificity[i] <- TN / (TN + FP)  # Specificity (True Negative Rate)
    
    cat("Class:", class, "\n")
    cat("Sensitivity:", sensitivity[i], "\n")
    cat("Specificity:", specificity[i], "\n")
}

# Print overall sensitivity and specificity
cat("Overall Sensitivity:", mean(sensitivity), "\n")
cat("Overall Specificity:", mean(specificity), "\n")
```
...
```{r}
nb.fit <- naiveBayes(formula(multinom_model), data = train_data)
nb.fit
```
...
```{r}
nb.pred <- predict(nb.fit, newdata = test_data, type = "raw")
```
...
# Predictions (raw probabilities for all classes)
nb.pred <- predict(nb.fit, newdata = test_data, type = "raw")

# Ensure FTR is a factor in the test data
test_data$FTR <- as.factor(test_data$FTR)

# Validate response and predictor alignment
if (!all(levels(test_data$FTR) %in% colnames(nb.pred))) {
  stop("Mismatch between levels of test_data$FTR and columns in nb.pred")
}

# Calculate predicted classes by selecting the class with highest probability
predicted_classes <- apply(nb.pred, 1, which.max)
predicted_classes <- levels(test_data$FTR)[predicted_classes]  # Convert indices to class labels

# Create confusion matrix
confusion_matrix <- confusionMatrix(factor(predicted_classes, levels = levels(test_data$FTR)), test_data$FTR)
print("Confusion Matrix:")
print(confusion_matrix)
...
# Loop through each class and generate individual ROC curves
roc_curves <- list()

for (class in levels(test_data$FTR)) {
  # Create a binary response for the current class
  binary_response <- ifelse(test_data$FTR == class, 1, 0)
  
  # Compute the ROC curve
  roc_curve <- roc(binary_response, nb.pred[, class])
  roc_curves[[class]] <- roc_curve
  ...
  plot.roc(
    roc_curve, 
    lwd = 2, 
    main = paste("ROC Curve for Class:", class),
    print.auc = TRUE
  )
