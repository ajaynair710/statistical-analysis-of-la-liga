...
# If it's a single vector, you can use it directly
if (is.vector(model_fit.ridge.pred)) {
    # If it's a binary classification, you might want to convert it to a factor
    model_fit.ridge.pred <- as.numeric(model_fit.ridge.pred > 0.5)  # Assuming 0.5 is the threshold
}
...
cat("Length of model_fit.ridge.pred:", length(model_fit.ridge.pred), "\n")

# Check contents
print(head(y_test))
print(head(model_fit.ridge.pred))

# Apply the roc function to get the best threshold value
if (length(y_test) == length(model_fit.ridge.pred)) {
    model_fit.ridge.roc <- roc(y_test, model_fit.ridge.pred, plot = TRUE, print.auc = TRUE)

    # Get the best threshold
    best_threshold <- coords(model_fit.ridge.roc, "best", ret = "threshold")
    print(best_threshold)
} else {
    cat("Error: Lengths of response and predictions do not match. Cannot compute ROC.\n")
}
```