...
In this section, we'll apply the Naive Bayes algorithm to the dataset. We'll first train the model using the training data and assess its performance.

```{r}
# Load required package
require(e1071)

# Convert target variable (FTR) to factor
train_data$FTR <- factor(train_data$FTR, levels = c("Home Win", "Away Win", "Draw"))

# Train the Naive Bayes model
naive_bayes_model <- naiveBayes(FTR ~ HST + AST + HF + AF + HC + AC + HY + AY + HR + AR, data = train_data)
...
Now that we have trained the Naive Bayes model, we can use it to predict the outcomes for the test dataset.

```{r}
# Prediction on test data
test_output <- test_data
test_output$Predicted <- predict(naive_bayes_model, newdata = test_output)
test_output <- test_output[c(1:4, 17)]
names(test_output)[4] <- "Actual"

# Save prediction output as CSV file
write.csv(test_output, "./testing/naive-bayes-output.csv", row.names = FALSE)
```

## Model Evaluation

We'll now assess the model's performance using a confusion matrix and other evaluation metrics.

```{r}
# Convert predicted and actual values to factors
predicted_factor <- factor(test_output$Predicted, levels = c("Home Win", "Away Win", "Draw"))
actual_factor <- factor(test_data$FTR, levels = c("Home Win", "Away Win", "Draw"))
...
```{r}
# Save the model object to a file
saveRDS(naive_bayes_model, "./model/naive_bayes_model_file.rds")
```